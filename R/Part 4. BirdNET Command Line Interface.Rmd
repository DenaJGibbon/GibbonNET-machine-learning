...
# List of folds to loop through
FOLDS=(
    "/Volumes/DJC Files/OrxyGibbonAutomatedDetection/KFolds/fold_1"
    "/Volumes/DJC Files/OrxyGibbonAutomatedDetection/KFolds/fold_2"
    "/Volumes/DJC Files/OrxyGibbonAutomatedDetection/KFolds/fold_3"
    "/Volumes/DJC Files/OrxyGibbonAutomatedDetection/KFolds/fold_4"
    "/Volumes/DJC Files/OrxyGibbonAutomatedDetection/KFolds/fold_5"
)

# Loop over each fold
for FOLD in "${FOLDS[@]}"; do
    TRAIN_FOLDER="$FOLD/train"  # Path to the 'train' folder in each fold
    
    if [ -d "$TRAIN_FOLDER" ]; then
        FOLDER_NAME=$(basename "$FOLD")
        OUTPUT_PATH="$FOLD/${FOLDER_NAME}.tflite"  # Save the model in the same fold directory
        
        # Run the training script using the data from the 'train' folder
        python train.py --i "$TRAIN_FOLDER" --o "$OUTPUT_PATH" --fmin 500 --fmax 3000 && \
        echo "Processed: $FOLDER_NAME and saved model in $OUTPUT_PATH"
    else
        echo "Train folder not found for fold: $FOLDER_NAME"
    fi
done
