...
TestDataSet  <- list.files('/Users/denaclink/Desktop/BirdNETOutput_automated_detection/AnnotatedFiles/',
                              full.names = TRUE)


# Get a list of annotation selection table files
TopModelresults <- list.files('/Users/denaclink/Desktop/BirdNETOutput_automated_detection/fold_1/',
                          full.names = TRUE)

TopModelresults  <- list.files('/Volumes/DJC Files/OrxyGibbonAutomatedDetection/TestResults/BirdNETOutput_automated_detection/Maliau_binary',
                           full.names = TRUE)
...
for (f in 1:length(TopModelresults)) {
  ...
  if(length(testDataIndex) > 0 ){
    ...
    for (c in 1:nrow(TempTopModelTable)) {
      ...
      if (!is.na(TempRow$Begin.Time..s.)) {
        ...
        if (nrow(matched_detections) > 0) {
          ...
          TempRow$signal <- TempRow$Species.Code
    ...
    if (length( unlist(DetectionList)) > 0 &  length( unlist(DetectionList)) < nrow(TestDataTable) ) {
      ...
      missed_detections$signal <- 'HF_FG'
...
for(a in 1:length(Thresholds)){
  ...
  TopModelDetectionDF_single$Predictedsignal <-
    ifelse(TopModelDetectionDF_single$Confidence  <=Thresholds[a], 'noise','HF_FG')
  
  TopModelDetectionDF_single$Predictedsignal <- ifelse(TopModelDetectionDF_single$Predictedsignal=='noise','noise','HF_FG')
  TopModelDetectionDF_single$signal <- ifelse(TopModelDetectionDF_single$signal=='noise','noise','HF_FG')
  ...
  caretConf <- caret::confusionMatrix(
    ...
    as.factor(TopModelDetectionDF_single$signal),positive = 'HF_FG',
    mode = 'everything')
